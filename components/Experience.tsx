        ...
        <div className="max-w-6xl mx-auto px-4 relative">
            ...
            <div className="mb-16 flex items-end gap-6 border-b border-white/20 pb-4">
                <div>
                    ...
                    <p className="font-mono text-sm mt-2" style={{ color: colors.textSecondary }}>// PROFESSIONAL_TRAJECTORY_LOG</p>
                </div>
            ...
            <div className="flex flex-col lg:flex-row gap-12 lg:gap-20">
                        ...
                        {EXPERIENCE.map((exp) => (
                            ...
                            >
                                ...
                                <div
                                    ...
                                    style={selectedId === exp.id ? {
                                        backgroundColor: colors.grid,
                                        borderColor: colors.grid,
                                        boxShadow: `0 0 15px ${colors.grid}99`
                                    } : {}}
                                >
                                    {selectedId === exp.id && (
                                        <motion.div
                                            ...
                                            style={{ borderColor: `${colors.grid}4d` }}
                                        />
                                    )}
                                </div>
                                ...
                                <div
                                    className={`absolute left-[26px] top-3 h-px transition-all duration-300 ${selectedId === exp.id ? 'w-8 opacity-100' : 'w-0 opacity-0'}`}
                                    style={{ backgroundColor: `${colors.grid}80` }}
                                />
                                ...
                                <div className={`transition-all duration-300 ${selectedId === exp.id ? 'translate-x-4 opacity-100' : 'opacity-60 group-hover:opacity-100'}`}>
                                    ...
                                    <p className="font-mono text-xs tracking-wider mb-1" style={{ color: colors.textPrimary }}>{exp.period}</p>
                ...
                <div className="flex-1 min-h-[450px] relative">
                    ...
                    <svg className="absolute -left-16 top-8 w-16 h-full pointer-events-none hidden lg:block overflow-visible fill-none" style={{ stroke: `${colors.grid}4d` }}>
                        <path d="M 0 0 L 30 0 L 60 20" strokeDasharray="4 2" />
                        <circle cx="60" cy="20" r="2" style={{ fill: colors.grid }} />
                    </svg>
                            ...
                            <div className="flex items-start justify-between mb-8 border-b border-dashed border-white/20 pb-6 relative">
                                <div>
                                    <div className="flex items-center gap-2 mb-2">
                                        <Terminal className="w-4 h-4" style={{ color: colors.textPrimary }} />
                                        <span className="font-mono text-xs" style={{ color: colors.textPrimary }}>EXEC_ROLE: {selectedJob.id.toUpperCase()}</span>
                                    </div>
                                ...
                                <ul className="space-y-4 pl-6">
                                    {selectedJob.description.map((item, idx) => (
                                        <li key={idx} className="text-white/80 font-light leading-relaxed relative">
                                            <span className="absolute -left-6 top-2.5 w-1.5 h-1.5 rounded-full" style={{ backgroundColor: colors.textSecondary }} />
                                            {item}
                                        </li>
                                    ))}
                                </ul>
                            ...
                            <div className="mt-auto">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {selectedJob.technologies.map(tech => (
                                        <span
                                            key={tech}
                                            className="text-xs font-mono border border-white/10 bg-white/5 hover:bg-white/10 hover:border-white/30 transition-colors px-3 py-1.5 rounded-sm"
